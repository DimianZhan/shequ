...
<div class="col-md-9 topics-index main-col">
    ...
    @if (isset($category) && $category->id == config('phphub.qa_category_id'))
        <div class="alert alert-info">
            在 Dimianzhan，我们不提倡 <a href="{{ route('topics.show', 535) }}" style="text-decoration: underline;">新手提问</a> ，如果你编程遇到难题，请先 <a href="{{ route('topics.show', 3656) }}" style="text-decoration: underline;">搜索</a> 再 <a href="{{ route('topics.create', ['category_id' => config('phphub.qa_category_id')]) }}" class="btn btn-warning">提问</a>
        </div>
    @endif
    @if (isset($category) && $category->id === 1)
        <div class="alert alert-info">
            发布招聘贴前请必须仔细阅读 <a href="http://shequ.dimianzhan.com/topics/271" style="text-decoration: underline;">Dimianzhan社区 招聘贴发布规范</a>，不按规范发帖会被管理员永久下沉。<a href="{{ route('topics.create', ['category_id' => 1]) }}" class="btn btn-warning">发布招聘</a>
        </div>
    @endif
